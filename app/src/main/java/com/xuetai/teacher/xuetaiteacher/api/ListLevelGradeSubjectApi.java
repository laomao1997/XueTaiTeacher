...
public class ListLevelGradeSubjectApi {

    ApiInterface apiInterface;

    private ListLevelGradeSubjectApi() {
        // 创建 Retrofit 对象
    ...
    private static final class SingleHolder {
        private static final ListLevelGradeSubjectApi intance = new ListLevelGradeSubjectApi();
    }

    // 获取单例
    public static ListLevelGradeSubjectApi getInstance() {
    ...
    public void uploadResult(Subscriber<ResponseBody> subscriber) {
        JSONObject jsonParams = new JSONObject();
        JSONObject jsonObject = new JSONObject(); // 最外层的 JSONObeject 对象

        try {
            jsonObject.put("params", jsonParams);
            jsonObject.put("method", MethodCode.ListLevelGradeSubject);
            jsonObject.put("id", 1);
        } catch (Exception e) {
            KLog.e(e);
        }
